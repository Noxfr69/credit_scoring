...
class TestFlaskAPI(unittest.TestCase):
    ...
    def test_change_model_endpoint_200(self):
        ...
        response = requests.post('http://host.docker.internal:5001/new_model', data=json.dumps(data), headers={'Content-Type': 'application/json'})
    ...
    def test_predict_endpoint_with_valid_data(self):
        ...
        response = requests.post('http://host.docker.internal:5001/predict', data=data)
        self.assertEqual(response.status_code, 200)
    ...
    def test_get_dataset_version(self):
        response = requests.get('http://host.docker.internal:5001/version')
